...
○ a wrapper method called by the p_thread, which eventually invokes the
event_loop for that p_thread because p_thread calls a function of the signature
given below: void* event_loop_wrapper(void* arg)
...
○ Now in the int main, tid is an array of p_thread ids of type pthread_t of the size of
MAX_CLIENTS: pthread_t *tid = malloc(sizeof(pthread_t) * MAX_CLIENTS)
...
○ if everything goes well :-) assign a tid to the new client p_thread and invoke the
event_loop_wrapper routine with the new_s as parameter
○ then wait for all the clients to disconnect in order to stop the current server
session
